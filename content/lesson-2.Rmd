...
Now let's load the file `ofm_trend.Rmd` that you already downloaded into RStudio as our starting point. This `RMD` document should look somewhat familiar to the one we just did with Bob. It has a `yaml header`, a `code chunk`and some `inline text`. Lets take a look at the components quickly.
...
And finally let's calculate the `incporated share`
```{r eval=FALSE, echo=TRUE}
